...
void SMD15_init() // 要放在串口2初始化后面
{
    ...
    //	delay_ms(5);
    SMD15_setbaudrate(0);//设置波特率 设置成功后，重启雷达生效 一般不设置
    delay(100);
    //	//开始扫描
    start_scan();
    delay(100);
...
u8 get_front_message(void)
{
    ...
    front_dis = Sensor_Data[5] << 8 | Sensor_Data[4];
    engy = Sensor_Data[6];
    noise = Sensor_Data[7];
    memset(Sensor_Data, 0, 18); // 清一下数据
    return 1;
...
void getserial_task()
{
    while(FRONT_DIS.available())
    {
        Serial.println("okk");
        uint8_t getchar = FRONT_DIS.read();
        Serial.println(getchar);
        SDM15_Decode(getchar);
    }
}